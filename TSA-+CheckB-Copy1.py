...
index = pd.date_range(start="2017-12-19 23:10:00", periods=912, freq='0.5S')

#Please note that the above date range is simply a placeholder for analysis and is not same as timeseries time.

columns = ['Delta','Theta', 'Alpha', 'Beta']

split_Data = pd.DataFrame(index=index, columns=columns)

split_Data.set_index(index,inplace=True)
...
def bin_power(X,Band,Fs):
    ...
    for Freq_Index in range(0,len(Band)-1):
        ...
        #print floor(Freq/Fs*len(X))
        #print floor(Next_Freq/Fs*len(X))
...
read_Data = pd.read_csv("vf2.csv",parse_dates=["Time"],index_col="Time")
...
Filtered_Values = read_Data["Value"].values

#Fix Thresholds for Alpha, Beta, Theta, Delta 

Threshold=[0.5,4,7,12,30] 

# Set i iterator to 0 index
i = 0;

#Epoch Length ~400 values per 0.5 Seconds in the filtered set

epoch_length = 400;

#Calculate No of Iterations

n_epoch = len(Filtered_Values)/epoch_length;
print(int(n_epoch))

#Iterate through and find Bin_Power

split_Values = []

for x in range(0,int(n_epoch)):
    
    
    S1,S2=bin_power(Filtered_Values[i:epoch_length],Threshold,500);
    split_Values.append(S1)
    
    #increment to simulate split as batches for each second
    i = epoch_length+1;
    epoch_length = epoch_length+400;
    
split_Values

split_Data = pd.DataFrame(split_Values, index=index, columns=columns)
...
series = Series.from_csv('vf2.csv', header=0)
split_point = len(series) - 36107
...
groups = series_train['2017-12-21 23:10:00':'2017-12-21 23:21:00'].groupby(TimeGrouper('7T'))
